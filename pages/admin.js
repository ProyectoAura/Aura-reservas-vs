...
export default function Admin() {
    ...
    return (
      <main className="min-h-screen bg-[#0A1034] text-white p-6 font-sans">
        ...
        <div className="grid gap-4 max-w-md mx-auto">
          <button onClick={() => setSeccion("reservas")} className="bg-white/10 hover:bg-white/20 p-4 rounded-xl">📋 Reservas</button>
          <button onClick={() => setSeccion("menu")} className="bg-white/10 hover:bg-white/20 p-4 rounded-xl">🍽️ Menú</button>
          <button onClick={() => setSeccion("admin")} className="bg-white/10 hover:bg-white/20 p-4 rounded-xl">🛠️ Ingresar como Administrador</button>
        </div>
      </main>
    )
  }
  ...
  if (seccion === "menu") {
    return (
      <main className="min-h-screen bg-[#0A1034] text-white p-6 font-sans">
        <button onClick={() => setSeccion('principal')} className="text-[#D3C6A3] mb-4">← Volver</button>
  ...
  return (
    <main className="min-h-screen bg-[#0A1034] text-white p-6 font-sans">
      <button onClick={() => setSeccion("principal")} className="text-[#D3C6A3] mb-2">← Volver</button>
<h1 className="text-3xl text-center text-[#D3C6A3] mb-6 font-semibold">Panel del Administrador</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-w-2xl mx-auto mb-6">
        ...
        <button onClick={() => setVista("excel")} className="bg-white/10 hover:bg-white/20 py-3 rounded-xl">Descargar Excel Diario</button>
      ...
      {vista === "bloquear" && (
        <div className="bg-white/10 p-4 rounded-xl border border-white/20">
          <p>🔒 Función de bloqueo de horarios próximamente.</p>
        </div>
      )}

      {vista === "turnos" && (
        <div className="bg-white/10 p-4 rounded-xl border border-white/20">
          <p>🛠️ Panel para crear turnos aún en desarrollo.</p>
        </div>
      )}

      {vista === "estadisticas" && (
        <div className="bg-white/10 p-4 rounded-xl border border-white/20">
          <p>📊 Módulo de estadísticas estará disponible en breve.</p>
        </div>
      )}

      {vista === "excel" && (
        <div className="text-center text-[#D3C6A3] font-semibold">📥 El Excel diario se descargó automáticamente.</div>
      )}
    </main>
  );
}
